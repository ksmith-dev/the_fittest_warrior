    ...
    <div id="admin">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                @if(Auth::user()->role === 'admin')
                    ...
                    @endif
                    @if(Request::url() == url('view/stay_informed'))
                        <li class="breadcrumb-item active" aria-current="page">stay informed</li>
                    @else
                        <li class="breadcrumb-item"><a href="{{ url('view/stay_informed') }}">stay informed</a></li>
                    @endif
                @else
        ...
        <div class="row">
            @if(Auth::user()->role === 'admin')
                ...
                <div class="spacer-20"></div>
                <a class="btn btn-warning" href="{{ url('view/stay_informed') }}" role="button" style="width: 100%">MANAGE STAY INFORMED LIST</a>
                ...
                <div class="col">
                    @if(!empty($param['models']))
                        @if($param['models']->count() < 1)
                            ...
                            <h5 style="width: 80%">This is not a reflection on you, this just means that we do not have any stored records. If you want to store some {{ $param['model_type'] }} records please start by clicking the add {{ $param['model_type'] }} button.</h5>
                            <div class="spacer-20"></div>
                        @endif
                    @endif
                    <div class="spacer-20"></div>
            ...
            @elseif($param['page_type'] === 'stay_informed')
                <div class="col">
                    @if(!empty($param['models']))
                        @if($param['models']->count() < 1)
                            <div class="spacer-50"></div>
                            <h2 style="width: 90%;">Welcome to your {{ str_replace('_', ' ', $param['model_type']) }} tracker, there are no records to display.</h2>
                            <span class="spacer-50"></span>
                            <h5 style="width: 80%">This is not a reflection on you, this just means that we do not have any stay informed emails to display.</h5>
                            <div class="spacer-20"></div>
                        @else
                            <table class="table">
                                <thead>
